...
class MltCnnL4ORI(nn.Module): # revised to less feature maps ( / 2)
    ...
    def __init__(self, block, num_blocks, num_cls_l1=2, num_cls_l2=3, num_cls_l3=4, num_cls_l4=6):
        ...
        self.branch4 = nn.Sequential(nn.Linear(256*8*8+8, num_cls_l4))
    ...
    def forward(self, x, poc, qp, n0, n1, n2, n3, n4, n5):
        ...
        lvl1 = torch.cat([lvl1, poc, qp, n0, n1+n2+n3+n4+n5], dim=1)
        lvl1 = self.branch1(lvl1)
        out = self.layer2(out)
        # --- Second branch --- #
        lvl2 = out.view(out.size(0), -1)
        lvl2 = torch.cat([lvl2, poc, qp, n0, n1, n2+n3+n4+n5], dim=1)
        lvl2 = self.branch2(lvl2)
        out = self.layer3(out)
        # --- Third branch --- #
        lvl3 = out.view(out.size(0), -1)
        lvl3 = torch.cat([lvl3, poc, qp, n0, n1, n2+n3, n4+n5], dim=1)
        lvl3 = self.branch3(lvl3)
        out = self.layer4(out)
        # --- Fourth branch --- #
        lvl4 = out.view(out.size(0), -1)
        lvl4 = torch.cat([lvl4, poc, qp, n0, n1, n2, n3, n4, n5], dim=1)
        lvl4 = self.branch4(lvl4)

        return lvl1, lvl2, lvl3, lvl4

def MltCuORI():
    return MltCnnL4ORI(BasicBlock, [2, 2, 2, 2])
