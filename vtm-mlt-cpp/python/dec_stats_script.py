...
for n, fn in enumerate(fnames):
    ...
    seq_name = seg2[0]
    script = open('../script_resnetQP/%s_stats.sh' % seq_name, 'w')
    script.writelines('#! /bin/bash\n\n\n')
    for i, q in enumerate(qp):
        result_name = seq_name + '_q' + str(q)
        script.writelines('../bin/DecoderAppStatic '
                          '-b ../ENC_RESULT_resnetQP/RA/bin/%s.bin '
                          '--TraceFile=../STATS_RESULT_resnetQP/RA/trace/%s.txt --TraceRule=D_SYNTAX,D_HEADER:final==1'
                          '&> ../STATS_RESULT_resnetQP/RA/log/%s.txt\n' % (result_name, result_name, result_name))
